...
check "POST" "200" "$(curl -s -o /dev/null -w '%{http_code}' -X POST http://127.0.0.1:$PORT/)"
check "PUT" "200" "$(curl -s -o /dev/null -w '%{http_code}' -X PUT http://127.0.0.1:$PORT/)"
check "DELETE" "200" "$(curl -s -o /dev/null -w '%{http_code}' -X DELETE http://127.0.0.1:$PORT/)"
check "OPTIONS" "200" "$(curl -s -o /dev/null -w '%{http_code}' -X OPTIONS http://127.0.0.1:$PORT/)"
check "CONNECT" "200" "$(status 'CONNECT / HTTP/1.1\r\nHost: x\r\n\r\n')"
check "TRACE" "200" "$(status 'TRACE / HTTP/1.1\r\nHost: x\r\n\r\n')"
...
check "valid Content-Length" "200" "$(status 'POST / HTTP/1.1\r\nHost: x\r\nContent-Length: 4\r\n\r\ntest')"
...
check "body content" "Hello, World!!" "$(curl -s http://127.0.0.1:$PORT/)"
